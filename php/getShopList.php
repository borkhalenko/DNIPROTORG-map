<?php
   ...
   $sql = "SELECT contractor_name, contractor_address, tt_number, coords FROM `contractors` WHERE tt_number!=0 GROUP BY tt_number ORDER BY tt_number";
    if (!$result=mysql_query($sql,$link)){
        echo "Cannot select route_id from routes";
        mysql_close($link);
        exit();
    }
    while($parsedResult=mysql_fetch_array($result)){
